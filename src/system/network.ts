...
export function ensureChains(): ResultAsync<void, LobsterError> {
  ...
  return execUnchecked(['iptables', '-N', CHAIN_INPUT])
    ...
    .andThen(() => execUnchecked(['iptables', '-C', 'INPUT', '-j', CHAIN_INPUT]))
    .andThen((r) => {
      if (r.exitCode !== 0) {
        return exec(['iptables', '-I', 'INPUT', '1', '-j', CHAIN_INPUT]).map(() => undefined);
      }
      return okAsync(undefined);
    })
    .andThen(() => execUnchecked(['iptables', '-C', 'FORWARD', '-j', CHAIN_FORWARD]))
    .andThen((r) => {
      if (r.exitCode !== 0) {
        return exec(['iptables', '-I', 'FORWARD', '1', '-j', CHAIN_FORWARD]).map(() => undefined);
      }
      return okAsync(undefined);
    })
    .map(() => undefined);
...
export function createTap(name: string, hostIp: string, guestIp: string): ResultAsync<void, LobsterError> {
  ...
  return exec(['ip', 'tuntap', 'add', 'dev', name, 'mode', 'tap'])
    ...
    .andThen(() => exec(['sysctl', '-w', `net.ipv6.conf.${name}.disable_ipv6=1`]))
    .map(() => undefined)
    .mapErr((e) => ({
      ...e,
...
export function addIsolationRules(tapName: string): ResultAsync<void, LobsterError> {
  ...
  return exec([
    'iptables', '-A', CHAIN_INPUT, '-i', tapName,
    ...
    .andThen(() => exec([
      'iptables', '-A', CHAIN_FORWARD, '-o', tapName,
      '-m', 'conntrack', '--ctstate', 'ESTABLISHED,RELATED',
    ...
    .andThen(() => exec([
      'iptables', '-A', CHAIN_FORWARD, '-i', tapName,
    ...
    .andThen(() => exec([
      'iptables', '-A', CHAIN_FORWARD, '-i', tapName,
      '-m', 'connlimit', '--connlimit-above', String(CONNLIMIT_PER_TENANT), '--connlimit-saddr',
      '-j', 'DROP', ...comment('connlimit'),
    ]))
...
export function removeIsolationRules(tapName: string): ResultAsync<void, LobsterError> {
  const comment = (suffix: string) => ['-m', 'comment', '--comment', `lobster:${tapName}:${suffix}`];
  const ignore = () => okAsync({ exitCode: 0, stdout: '', stderr: '' } as const);

  return exec([
    'iptables', '-D', CHAIN_INPUT, '-i', tapName,
    '-m', 'conntrack', '--ctstate', 'ESTABLISHED,RELATED',
    '-j', 'ACCEPT', ...comment('host-return'),
  ]).orElse(ignore)
    .andThen(() => exec([
    ...
    .andThen(() => exec([
      'iptables', '-D', CHAIN_FORWARD, '-o', tapName,
      '-m', 'conntrack', '--ctstate', 'ESTABLISHED,RELATED',
      '-j', 'ACCEPT', ...comment('return'),
    ]).orElse(ignore))
    .andThen(() => exec([
    ...
    .andThen(() => exec([
      'iptables', '-D', CHAIN_FORWARD, '-i', tapName,
      '-d', '10.0.0.0/8', '-j', 'DROP', ...comment('block-rfc1918'),
    ]).orElse(ignore))
    .andThen(() => exec([
      'iptables', '-D', CHAIN_FORWARD, '-i', tapName,
